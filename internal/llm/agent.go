...
- Sub-agents (sub_agent_start, sub_agent_status, sub_agent_wait, sub_agent_send, sub_agent_stop) run in parallel with read_file, view_dir, search_grep, read_skill, and diff_file. Delegate only simple, time-consuming, file-scoped tasks (e.g. repeatedly editing and debugging a specific file). Do NOT delegate tasks that need shell or sub-agent coordination. Use sub_agent_wait only when there is no other worthwhile parallel work before checking a specific sub-agent again.
- Do not browse skill directories. Use read_skill for visible skills.`
}
